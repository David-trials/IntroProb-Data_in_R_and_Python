...
```{r load-data}
data(nycflights)
write.table(nycflights, file="Exported_nycflights.csv", sep=",") #Exporting the table to a file
```
...
</div>**The apparent distribution can be quite different depending on the bin size.**
...
sfo_feb_flights <- nycflights %>% filter(dest == "SFO", month == 2) #as before because it already creates a data frame in rstudio
dim(sfo_feb_flights)
```
...
<li> No flight is delayed more than 2 hours. </li> **THis one. There is a 196min delayed flight**
<li> The distribution has several extreme values on the right side. </li>
<li> More than 50% of flights arrive on time or earlier than scheduled. </li>**Note: 68% of data between +- 1 std**
</ol>
...
sfo_feb_flights %>% summarise(mean_ArrDly = mean(arr_delay), sd_ArrDly = sd(arr_delay),
                              median_ArrDly = median(arr_delay), IQR_ArrDly = IQR(arr_delay), n = n())
```
...
sfo_feb_flights %>% group_by(carrier) %>%
  summarise(median_ArrDly = median(arr_delay), IQR_ArrDly = IQR(arr_delay), n = n())
...
<li> January </li> 
<li> March </li> 
<li> July </li> **This one, just look at the table**
<li> October </li>
<li> December </li>
</ol>
...
<ol>
<li> January </li> **This one. See generated table below.**
<li> March </li> 
<li> July </li> 
<li> October </li>
<li> December </li>
...
<li> Median would be more reliable as the distribution of delays is skewed. </li> **this one**
...
<li> EWR </li> 
<li> JFK </li> 
<li> LGA </li> **This one has a higher percentage of on time flights ~72.8%**
</ol>
...
nycflights %>%  group_by(tailnum) %>%
  summarise(AveSpeed = avg_speed) %>%  arrange(desc(AveSpeed)) #or 
...
<li> There is an overall postive association between distance and average speed.</li> **True**
...
# Fraction that departed delayed but arrived on time
nycflights %>%  summarise( N_depDly = sum(dep_type == "delayed"), N_arrOT = sum(arr_type == "on time"),
                           N_dpDly_arrOT = sum(dep_type == "delayed" & arr_type == "on time" ), Ratio_Dly_OT = N_dpDly_arrOT / N_depDly  )
